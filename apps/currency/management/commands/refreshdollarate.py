...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        import requests
        self.stdout.write(bcolors.OKBLUE + "Getting currency info from cbu.uz..." + bcolors.ENDC)

        try:
            res = requests.get("https://cbu.uz/uz/arkhiv-kursov-valyut/json/")
            # response typeni python data typega o'giramiz
            data = res.json()

            # listdan birinchi elementni olamiz, sababi birinchi elementda dollar haqida malumot bor
            dollar = data[0]
            ...
            self.stdout.write(bcolors.WARNING + f"{e}" + bcolors.ENDC)
